test_that("summarize_pass_rate works correctly", {
  df <- create_pass_rate_test_df()

  # Test with no grouping variables
  result_no_group <- summarize_pass_rate(df, group_vars = NULL)
  ...
  result_grouped <- summarize_pass_rate(df, group_vars = "test_name")
  ...
  # Test error handling
  expect_error(summarize_pass_rate(1, "test_name"))
  expect_error(summarize_pass_rate(df, 1))
  expect_error(summarize_pass_rate(df, "test_name", "TRUE"))
})
...
test_that("summarize_performance_levels works correctly", {
  ...
  result_text <- summarize_performance_levels(df, group_vars = "test_name")
  ...
  result_numeric <- summarize_performance_levels(df, group_vars = "test_name", convert_performance_levels = FALSE, drop_parent_requested = FALSE)
